...
class CancelDetailsGuest extends Component {
	static propTypes = {
    ...
    const linkText = {
      ...
      cursor: 'pointer',
		...
		const { content: { guestName, hostName, confirmationCode, checkIn, checkOut, listTitle, listTimeZone, payoutToHost, currency, message, refundToGuest, cancellationPolicy, reservationId } } = this.props;
						...
						<TBody>
							<TR>
								<TD style={textStyle}>
									...
									<EmptySpace height={20} />
									<div>
                    We want to let you know that your reservation for {listTitle} from {checkInDate} - {checkOutDate} has been canceled.
								        {/* {
											refundToGuest > 0 && <span> As per your host's cancellation policy, you will be
								        		refunded <CurrencyView amount={refundToGuest} currency={currency} />.
								        	</span>
										}
										{
											refundToGuest === 0 && <span> As per your host's  cancellation policy, you will not
								        		receive any refund for this reservation.
								        	</span>
										} */}
                    {' '}You can view your total refund <a href={receiptURL} style={linkText}>here</a>, which is based on your host’s {cancellationPolicy} cancellation policy. 
                    We’re sorry it didn’t work out, and hope you’ll have the opportunity to take a Vegvisits trip soon!
									</div>
...
export default CancelDetailsGuest;
