...
import TeacherPhotosList from "../PhotosList/TeacherPhotosList";
  ...
  async complete(file) {
    ...
    if (file && file.xhr) {
      try {
        ...
        data = {
          ...data,
          photos: formTeachers[listId].photos.push({
            name: fileName,
            type: fileType
          })
        }
  ...
  render() {
    ...
    return (
        <div className={cx('listPhotoContainer')}>
          ...
          <TeacherPhotosList listId={listId} />
        </div>
    );
  }
...
export default withStyles(s)(connect(mapState, mapDispatch)(TeacherPhotoUpload));
