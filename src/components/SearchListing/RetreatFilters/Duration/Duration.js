    ...
    return (
      <div className={className}>
        <div ref={this.setBtnWrapperRef}>
          <Button
            className={cx({ [s.btnSecondary]: (isExpand === true || (durationRange && durationRange.length > 1)) }, s.btn)}
            onClick={() => handleTabToggle('duration', !isExpand)}>
            {
              !durationRange && <FormattedMessage {...messages.duration} />
            }
            {
              durationRange && durationRange.length > 1 && <span>
