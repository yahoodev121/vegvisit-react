...
class PhotosUpload extends Component {
    ...
    componentDidMount() {
        const {placeholder, field, listId} = this.props;
        ...
        if (placeholder) {
            this.setState({
                djsConfig: {
                    ...
                    hiddenInputContainer: '.' + field + '-' + listId,
                    // dictFileTooBig: '',
                }
            });
        }
    ...
    static getDerivedStateFromProps(props, state) {
        const {placeholder, field, listId} = props;
    ...
    async complete(file) {
        ...
        if (file && file.xhr) {
            try {
                ...
                const {formData, dispatch, field, listId} = this.props;
                if (field == 'photos') {
                    let formPhotos = formData.values.photos;
                    if (!formPhotos) formPhotos = [];
                    formPhotos.push({
                        name: fileName,
                        type: fileType
                    })
                    await dispatch(change('RetreatForm', 'photos', formPhotos));
    ...
    render() {
        const {placeholder, listId, field} = this.props;
        const {djsConfig} = this.state;
        ...
        return (
            <div className={cx('listPhotoContainer')}>
                <div className={cx('dzInputContainer', field + '-' + listId)}>
                    <MyDropZone
                ...
                <PhotosList listId={listId} field={field}/>
            </div>
        );
    }
...
function MyDropZone({config, eventHandlers, djsConfig}) {
    return (
        <DropzoneComponent
            config={config}
            eventHandlers={eventHandlers}
