...
class GuestRequirements extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    const { listingFields } = props;
    if (listingFields != undefined && !_.isEqual(listingFields.guestRequirements, state.guestRequirements)) {
      return { guestRequirements: listingFields.guestRequirements };
    } else {
      return null;
    }
  }
    ...
    return (
      <Grid fluid>
        <Row className={s.landingContainer}>
          ...
          <ListPlaceTips page={"guest-requirements"}/>
        </Row>
      </Grid>
    );
  }
